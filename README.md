...
│   ├── X.csv            # Gene expression matrix
│   ├── Y.csv            # Morphology embeddings / features
│   └── ...             
│
├── models/              # Encoder, decoder, discriminator modules
│   ├── encoders.py
│   ├── decoders.py
│   ├── discriminators.py
│   └── __init__.py
│
├── utils/               # Training utilities, losses, plotting helpers, etc.
│   ├── losses.py
│   ├── trainers.py
│   ├── eval_metrics.py
│   └── visualization.py
...
│   ├── geo_advae_default.yaml
│   └── ...
│
├── analysis/            # Downstream analysis and visualization
│   ├── plot_umap.py
│   ├── clustering.py
│   └── ...
│
...
Place your preprocessed data files in the `data/` directory, for example:

- `data/X.csv` – gene expression data (cells × genes)  
- `data/Y.csv` – morphology embeddings or features (cells × morphology features)

---

### 2. Training the Model

To train GeoAdVAE with a configuration file (e.g., `configs/geo_advae_default.yaml`):

```bash
python train.py --config configs/geo_advae_default.yaml
```
...
After training, you can compute embeddings, perform clustering, and generate UMAP visualizations:

```bash
python analysis/plot_umap.py     --embedding_path results/geo_advae_latent.npy     --output_path figures/geo_advae_umap.png
```
...
To reproduce the exact environment used for the experiments:
