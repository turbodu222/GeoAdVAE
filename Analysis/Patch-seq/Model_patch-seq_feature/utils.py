...
def generate_color_pairs_with_celltype_mapping(unique_labels):
    ...
    Returns dictionary mapping labels to (morph_color, gex_color) tuples
    where morph_color is the lighter color for morphology
    and gex_color is the darker color for gene expression
    """
    import matplotlib.colors as mcolors
    import numpy as np
    
    # Gene expression colors (darker, for GEX)
    # Based on cluster_cols_gex from R
    gex_colors = {
        'IT': '#ED3535',
        'ET': '#F47506',
        'CT': '#DF659C',
    ...
    # Morphology colors (lighter, for morphology)
    # Based on cluster_cols_morph from R
    morph_colors = {
        'IT': '#F29E9E',
        'ET': '#F6B48F',
        'CT': '#E6A0C4',
    ...
    # Default colors for unknown cell types
    default_gex_colors = [
    ...
    for label in unique_labels:
        if label in gex_colors and label in morph_colors:
            # Use predefined colors for known cell types
            gex_color = gex_colors[label]
            morph_color = morph_colors[label]
        else:
            # Use default colors for unknown labels
            base_color = default_gex_colors[default_color_idx % len(default_gex_colors)]
            default_color_idx += 1
            
            # GEX color is the base color
            gex_color = base_color
            
            # Morphology color is a lighter version
            rgb = mcolors.hex2color(base_color)
            morph_rgb = tuple(0.4 * c + 0.6 for c in rgb)  # Mix with 60% white
            morph_color = mcolors.to_hex(morph_rgb)
        
        # Return as (morph_color, gex_color)
        # morph_color = lighter color for morphology
        # gex_color = darker color for gene expression
        color_pairs[label] = (morph_color, gex_color)
...
def save_plots(trainer, data_a, data_b, directory, suffix, config=None):
    """
    Backward compatible wrapper for save_plots_with_celltype_colors
    This function is called by train.py
    """
    # Call the full version with dataset=None (will be handled internally)
    save_plots_with_celltype_colors(
        trainer, data_a, data_b, directory, suffix, config=config, dataset=None
    )