...
def write_knn(trainer, data_a, data_b, directory, suffix, dataset=None):
    """
    Updated for unbalanced data: No pairing accuracy calculation
    Only logs latent space statistics for monitoring
    """
    ...
    output_lines.append(f"=== Unbalanced Dataset Evaluation ===")
    output_lines.append(f"Morphology latent shape: {latent_a.shape}")
    output_lines.append(f"Gene expression latent shape: {latent_b.shape}")
    
    # Calculate latent space statistics
    output_lines.append(f"\nLatent Space Statistics:")
    output_lines.append(f"Morphology - mean: {latent_a.mean():.4f}, std: {latent_a.std():.4f}")
    output_lines.append(f"Gene Expression - mean: {latent_b.mean():.4f}, std: {latent_b.std():.4f}")
    
    # Note: Pairing accuracy removed because data is not paired (645 morpho vs 1329 GEX)
    output_lines.append(f"\nNote: Pairing accuracy metrics not computed for unbalanced dataset")
    
    if dataset is not None:
        rna_labels_dict = dataset.get_rna_family_labels()
        if rna_labels_dict is not None:
            rna_morpho = rna_labels_dict.get('morpho')
            rna_gex = rna_labels_dict.get('gex')
            
            if rna_morpho is not None:
                unique_morpho = len(np.unique(rna_morpho))
                output_lines.append(f"Morphology RNA types: {unique_morpho}")
            
            if rna_gex is not None:
                unique_gex = len(np.unique(rna_gex))
                output_lines.append(f"Gene Expression RNA types: {unique_gex}")
    ...
    # Write to file
    with open(os.path.join(directory, "evaluation_log.txt"), "a") as myfile:
        for line in output_lines:
            myfile.write(line + "\n")
...
def get_rna_family_colors():
    """
    Get fixed color mappings for RNA families
    Returns: (morpho_color_map, gex_color_map) dictionaries
    """
    # Morphology colors (pastel/light colors)
    cluster_cols_morph = {
        "IT": "#F29E9E",
        "ET": "#F6B48F",
        "CT": "#E6A0C4",
    ...
    return cluster_cols_morph, cluster_cols_gex
...
def plot_latent_visualization(a, b, outname1=None, outname2=None, outname=None, 
                            ...
                            save_coordinates=True, coord_dir=None, suffix="",
                            dataset=None):
    """
    Updated plotting function for unbalanced data with separate RNA family labels
    
    Args:
        a: Morphology latent representations (645 samples)
        b: Gene expression latent representations (1329 samples)
        ...
        dataset: Dataset object with RNA family labels (optional)
    """
    import matplotlib.pyplot as plt
    from sklearn.decomposition import PCA
    from sklearn.preprocessing import StandardScaler
    ...
    # Get separate RNA family labels for each modality
    labels_morpho = None
    labels_gex = None
    ...
    if dataset is not None:
        rna_labels_dict = dataset.get_rna_family_labels()
        if rna_labels_dict is not None:
            labels_morpho = rna_labels_dict.get('morpho')
            labels_gex = rna_labels_dict.get('gex')
    
    # Get actual sample counts (unbalanced)
    n_morpho = len(a)  # 645 morphology samples
    n_gex = len(b)     # 1329 gene expression samples
    
    print(f"\n=== Unbalanced Data Visualization ===")
    print(f"Morphology samples: {n_morpho}")
    print(f"Gene expression samples: {n_gex}")
    ...
    # Split coordinates back to modalities
    # comp[:n_gex] = gene expression coordinates
    # comp[n_gex:] = morphology coordinates
    gex_coords = comp[:n_gex]
    morpho_coords = comp[n_gex:]

    # === Save coordinates to CSV ===
    if save_coordinates and coord_dir is not None:
        print(f"\n=== Saving {method.upper()} coordinates to CSV ===")
        
        # Create coordinate directory if it doesn't exist
        if not os.path.exists(coord_dir):
            os.makedirs(coord_dir)
            print(f"Created coordinate directory: {coord_dir}")
        
        # Save Gene Expression coordinates
        gex_df = pd.DataFrame({
            'Sample_ID': range(n_gex),
            'Coord_1': gex_coords[:, 0],
            'Coord_2': gex_coords[:, 1],
            'Modality': 'Gene_Expression'
        })
        
        if labels_gex is not None:
            gex_df['RNA_Family'] = labels_gex[:n_gex]
        
        gex_csv_path = os.path.join(coord_dir, f"GEX_{method}_coordinates_{suffix}.csv")
        gex_df.to_csv(gex_csv_path, index=False)
        print(f"✓ Saved GEX coordinates: {gex_csv_path}")
        print(f"  Samples: {len(gex_df)}")
        
        # Save Morphology coordinates
        morpho_df = pd.DataFrame({
            'Sample_ID': range(n_morpho),
            'Coord_1': morpho_coords[:, 0],
            'Coord_2': morpho_coords[:, 1],
            'Modality': 'Morphology'
        })
        
        if labels_morpho is not None:
            morpho_df['RNA_Family'] = labels_morpho[:n_morpho]
        
        morpho_csv_path = os.path.join(coord_dir, f"Morpho_{method}_coordinates_{suffix}.csv")
        morpho_df.to_csv(morpho_csv_path, index=False)
        print(f"✓ Saved Morpho coordinates: {morpho_csv_path}")
        print(f"  Samples: {len(morpho_df)}")
        
        # Save combined coordinates
        combined_df = pd.concat([gex_df, morpho_df], ignore_index=True)
        combined_csv_path = os.path.join(coord_dir, f"Combined_{method}_coordinates_{suffix}.csv")
        combined_df.to_csv(combined_csv_path, index=False)
        print(f"✓ Saved combined coordinates: {combined_csv_path}")
        print(f"  Total samples: {len(combined_df)} (GEX: {n_gex}, Morpho: {n_morpho})")
        print("="*60)
    
    # Create color mapping for RNA families using fixed colors
    # Get predefined color maps for morphology and GEX
    color_map_morpho, color_map_gex = get_rna_family_colors()
    
    all_labels = []
    if labels_morpho is not None:
        all_labels.extend(list(labels_morpho))
    if labels_gex is not None:
        all_labels.extend(list(labels_gex))
    
    if len(all_labels) > 0:
        unique_labels = np.unique(all_labels)
        n_unique = len(unique_labels)
        
        # Use predefined colors from the color maps
        # Morphology uses pastel colors, GEX uses solid colors
        print(f"Found {n_unique} unique RNA family labels: {unique_labels}")
        print(f"Using predefined color scheme for RNA families")
    else:
        unique_labels = None
        color_map_morpho = {}
        color_map_gex = {}
        print("No RNA family labels found, using default colors")

    # Plot Morphology only (pastel filled circles)
    if outname1:
        fig, ax = plt.subplots(figsize=(10, 8))
        if labels_morpho is not None:
            for label in np.unique(labels_morpho):
                mask = labels_morpho == label
                if np.any(mask):
                    # Use predefined morphology color, fallback to gray if not found
                    color = color_map_morpho.get(label, '#CCCCCC')
                    ax.scatter(morpho_coords[mask, 0], morpho_coords[mask, 1], 
                             c=color, s=60, marker='o',
                             alpha=0.7, edgecolors='none', 
                             label=f'{label}')
        else:
            ax.scatter(morpho_coords[:, 0], morpho_coords[:, 1], s=60, marker='o',
                     c='lightblue', alpha=0.7, edgecolors='none', 
                     label='Morphology')
        
        ax.set_xlabel(f"{method.upper()} 1")
        ax.set_ylabel(f"{method.upper()} 2")
        ax.set_title(f"Morphology Embedding ({method.upper()}, n={n_morpho})")
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.savefig(outname1, dpi=300, bbox_inches='tight')
        plt.close()

    # Plot Gene Expression only (solid empty triangles)
    if outname2:
        fig, ax = plt.subplots(figsize=(10, 8))
        if labels_gex is not None:
            for label in np.unique(labels_gex):
                mask = labels_gex == label
                if np.any(mask):
                    # Use predefined GEX color, fallback to gray if not found
                    color = color_map_gex.get(label, '#666666')
                    ax.scatter(gex_coords[mask, 0], gex_coords[mask, 1], 
                             s=60, marker='^',
                             facecolors='none', edgecolors=color, 
                             linewidth=2, alpha=0.8, label=f'{label}')
        else:
            ax.scatter(gex_coords[:, 0], gex_coords[:, 1], s=60, marker='^',
                     facecolors='none', edgecolors='darkorange', 
                     linewidth=2, alpha=0.8, label='Gene Expression')
        
        ax.set_xlabel(f"{method.upper()} 1")
        ax.set_ylabel(f"{method.upper()} 2")
        ax.set_title(f"Gene Expression ({method.upper()}, n={n_gex})")
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.savefig(outname2, dpi=300, bbox_inches='tight')
        plt.close()
    ...
    if outname:
        ...
        if unique_labels is not None:
            # Plot each RNA family type
            for label in unique_labels:
                # Morphology (pastel filled circles)
                if labels_morpho is not None:
                    mask_morpho = labels_morpho == label
                    if np.any(mask_morpho):
                        color = color_map_morpho.get(label, '#CCCCCC')
                        ax.scatter(morpho_coords[mask_morpho, 0], morpho_coords[mask_morpho, 1], 
                                 c=color, s=60, marker='o',
                                 alpha=0.7, edgecolors='none', 
                ...
                if labels_gex is not None:
                    mask_gex = labels_gex == label
                    if np.any(mask_gex):
                        color = color_map_gex.get(label, '#666666')
                        ax.scatter(gex_coords[mask_gex, 0], gex_coords[mask_gex, 1], 
                                 s=60, marker='^',
                                 facecolors='none', edgecolors=color, 
            ...
            ax.scatter(morpho_coords[:, 0], morpho_coords[:, 1], s=60, marker='o',
                     c='lightblue', alpha=0.7, edgecolors='none', 
                     label=f'Morphology (n={n_morpho})')
            ax.scatter(gex_coords[:, 0], gex_coords[:, 1], s=60, marker='^',
                     facecolors='none', edgecolors='darkorange', 
                     linewidth=2, alpha=0.8, label=f'Gene Expression (n={n_gex})')
        
        ax.set_xlabel(f"{method.upper()} 1")
        ax.set_ylabel(f"{method.upper()} 2")  
        ax.set_title(f"Combined Morphology ({n_morpho}) and Gene Expression ({n_gex}) ({method.upper()})")
        ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.savefig(outname, dpi=300, bbox_inches='tight')
        plt.close()
...
def save_plots(trainer, data_a, data_b, directory, suffix, config=None, dataset=None):
    """
    Updated save_plots function for unbalanced data with dataset support
    """
    latent_a = trainer.gen_a.enc(data_a).data.cpu().numpy()
    latent_b = trainer.gen_b.enc(data_b).data.cpu().numpy()
    ...
    # === NEW: Create coordinates directory ===
    coord_dir = os.path.join(directory, 'coordinates')
    
    # Use the new visualization function with CSV export and dataset support
    plot_latent_visualization(
        ...
        save_coordinates=True,    # === NEW ===
        coord_dir=coord_dir,      # === NEW ===
        suffix=suffix,            # === NEW ===
        dataset=dataset           # === NEW ===
    )
    
    print(f"✓ Saved plots and coordinates for iteration {suffix}")  # === NEW ===