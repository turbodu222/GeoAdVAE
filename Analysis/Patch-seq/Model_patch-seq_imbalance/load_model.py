...
def load_data(isatac=False, data_size=1874, for_training=True, supervise=[], drop=False):
    """
    Load .npz data (we don't use .csv files here)
    Args:
        isatac (bool): If True, load ATAC-seq data; otherwise, load expression data.
        data_size (int): Size of the data to load.
        for_training (bool): If True, apply log transformation to the data.
        supervise (list): List of labels for supervised learning.
        drop (bool): If True, drop features with low variance.
    Returns:
...
def get_unit_model_output(data_a, data_b, name):
    """
    Load the model and get the latent representations for the given data.
    Args:
        data_a (torch.Tensor): ATAC-seq data.
        data_b (torch.Tensor): Expression data.
        name (str): Name of the model configuration to load.
    Returns:
        tuple: Latent representations for ATAC-seq and expression data.
    """
...
def main():
    data_a = load_data_from_csv(is_modality1=True)
    data_b = load_data_from_csv(is_modality1=False)
