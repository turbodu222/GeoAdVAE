...
while True:
    for it, batch in enumerate(train_loader):
        ...
        # Extract batch data (using separate indices for unbalanced data)
        images_a = batch['morpho_data'].to(device)
        images_b = batch['gex_data'].to(device)
        indices_a = batch['morpho_index'].cpu().numpy()  # Morphology indices (cycling through 645)
        indices_b = batch['gex_index'].cpu().numpy()     # GEX indices (up to 1329)
        ...
        if iterations % 500 == 0:
            ...
            for i in range(min(3, len(indices_a))):
                morpho_idx = indices_a[i]
                gex_idx = indices_b[i]
                morpho_cluster = morpho_clusters[i]
                gex_cluster = gex_clusters[i]
                rna_family_morpho = batch['rna_family_morpho'][i] if batch['rna_family_morpho'][i] is not None else "N/A"
                rna_family_gex = batch['rna_family_gex'][i] if batch['rna_family_gex'][i] is not None else "N/A"
                print(f"Batch sample {i}: "
                      f"Morpho[{morpho_idx}] cluster={morpho_cluster} RNA={rna_family_morpho}, "
                      f"GEX[{gex_idx}] cluster={gex_cluster} RNA={rna_family_gex}")
            sys.stdout.flush()
        ...
        if (iterations + 1) % config['snapshot_save_iter'] == 0:
            ...
            save_plots(trainer, data_a, data_b, image_directory, str(iterations), config, dataset)
        ...
        if iterations >= max_iter:
            ...
            save_plots(trainer, data_a, data_b, image_directory, "final", config, dataset)
