...
df <- read.csv("/Users/kevinlin/Library/CloudStorage/Dropbox/Collaboration-and-People/turbo/git/morpho_integration/csv/kevin/Writeup6_turbo-csvs/5xFAD_GeoAdvAE_gene_importance_ranked.csv",
               row.names = 1)
...
gse_df2 <- gse_df[gse_df$p.adjust <= 0.05,]
...
go_red  <- c("GO:0001906")   # cell killing
go_blue <- c("GO:0006281")   # DNA repair
...
plot1 <- ggplot(df, aes(order, value)) +
  ...
  labs(x = "Rank (decreasing value)", y = "teststat",
       ...
       subtitle = "red: cell killing • blue: DNA repair • black: other") +
  theme_minimal(base_size = 12)

ggplot2::ggsave(plot1,
                file = paste0(plot_folder, "Writeup6_gsea.png"),
                height = 5, width = 8)
...
ggplot2::ggsave(plot1,
                file = paste0(plot_folder, "Writeup6_gsea_cleaned.png"),
                height = 2.5, width = 5)
