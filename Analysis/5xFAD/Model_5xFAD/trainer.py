...
class Trainer(nn.Module):
    ...
    def gen_update(self, x_a, x_b, hyperparameters, variational=True, 
        ...
        """
        Enhanced generator update with conditional loss computation
        """
        # Skip debug if GAN is disabled
        if hyperparameters.get('gan_w', 1.0) > 0 and self.debug_mode and self.current_iteration % 100 == 0:
        ...
        # Get loss weights
        kl_weight = hyperparameters.get('kl_w', 1.0)
        recon_weight = hyperparameters['recon_x_w']
        gan_weight = hyperparameters.get('gan_w', 1.0)
        gw_weight = hyperparameters.get('gw_w', 1.0)
        prior_weight = hyperparameters.get('lambda_p', 1.0)
        
        total_loss = torch.tensor(0.0, device=x_a.device)
        ...
        kl_loss = torch.tensor(0.0, device=x_a.device)
        recon_loss = torch.tensor(0.0, device=x_a.device)
        recon_loss_a = torch.tensor(0.0, device=x_a.device)
        recon_loss_b = torch.tensor(0.0, device=x_a.device)
        gan_loss = torch.tensor(0.0, device=x_a.device)
        gw_loss = torch.tensor(0.0, device=x_a.device)
        prior_loss = torch.tensor(0.0, device=x_a.device)
        
        # 1. KL Loss (skip computation if weight is 0)
        if kl_weight > 0 and variational and self._should_use_loss('kl_loss'):
        ...
        # 2. Reconstruction Loss (skip computation if weight is 0)
        if recon_weight > 0 and self._should_use_loss('recon_loss'):
        ...
        # 3. GAN Loss (skip entirely if weight is 0)
        if gan_weight > 0 and self._should_use_loss('gan_loss'):
        ...
        # 4. GW Loss (skip computation if weight is 0)
        if gw_weight > 0 and self._should_use_loss('gw_loss'):
            try:
                gw_mode = hyperparameters.get('gw_mode', 'groupwise')
                if gw_mode == 'global':
                    gw_loss = self._compute_global_gw_loss(z_a, z_b)
                else:
                    gw_group_size = hyperparameters.get('gw_group_size', 32)
                    gw_loss = self._compute_groupwise_gw_loss(z_a, z_b, gw_group_size)
                total_loss += gw_weight * gw_loss
            except Exception as e:
                print(f"GW loss computation failed: {e}")
        
        # 5. Prior Loss (skip entirely if weight is 0 or prior not available)
        if prior_weight > 0 and self._should_use_loss('prior_loss') and self.use_prior:
            try:
                prior_loss = self.compute_improved_prior_loss(z_a, z_b, batch_indices_a, batch_indices_b)
                total_loss += prior_weight * prior_loss
            except Exception as e:
        ...
        self.weighted_losses = {
            'kl_loss': kl_loss.item() * kl_weight if kl_weight > 0 and self._should_use_loss('kl_loss') else 0.0,
            'gw_loss': gw_loss.item() * gw_weight if gw_weight > 0 and self._should_use_loss('gw_loss') else 0.0,
            'recon_loss': recon_loss.item() * recon_weight if recon_weight > 0 and self._should_use_loss('recon_loss') else 0.0,
            'recon_loss_a': recon_loss_a.item() * recon_weight if recon_weight > 0 and self._should_use_loss('recon_loss') else 0.0,
            'recon_loss_b': recon_loss_b.item() * recon_weight if recon_weight > 0 and self._should_use_loss('recon_loss') else 0.0,
            'gan_loss': gan_loss.item() * gan_weight if gan_weight > 0 and self._should_use_loss('gan_loss') else 0.0,
            'prior_loss': prior_loss.item() * prior_weight if prior_weight > 0 and self._should_use_loss('prior_loss') else 0.0,
            'total_loss': total_loss.item()
        }
        ...
        # Skip debug verification if GAN is disabled
        if hyperparameters.get('gan_w', 1.0) > 0:
    ...
    def dis_update(self, x_a, x_b, hyperparameters):
        ...
        """
        # Skip entirely if GAN weight is 0
        if hyperparameters.get('gan_w', 1.0) == 0:
            return
