...
print("\n=== Cluster Index Check ===")
print(f"GEX clusters: {dataset.gex_cluster_labels.unique().tolist()}")
print(f"Morpho clusters: {dataset.morpho_cluster_labels.unique().tolist()}")
print(f"Prior matrix shape: {dataset.prior_matrix.shape if dataset.prior_matrix is not None else 'None'}")
print("="*30 + "\n")
...
while True:
    for it, batch in enumerate(train_loader):
        ...
        if iterations >= max_iter:
            ...
            write_knn(trainer, data_a, data_b, image_directory, "final", dataset, config)
